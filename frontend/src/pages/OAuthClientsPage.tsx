...
import { useOAuthClientsStore, type OAuthClient, type OAuthClientWithSecret } from '@/stores/oauthClientsStore';
import { useAuthStore } from '@/stores/authStore';
import { toast } from 'sonner';
import { Plus, Key, Copy, Check, Loader2, ExternalLink, Shield, Pencil, Trash2, RefreshCw } from 'lucide-react';

export function OAuthClientsPage() {
  const { clients, isLoading, error, fetchClients, createClient, updateClient, deleteClient, regenerateSecret } = useOAuthClientsStore();
  ...
  const [newClient, setNewClient] = useState<OAuthClientWithSecret | null>(null);
  ...
  const handleCreate = async () => {
    ...
    if (!isInternal) {
      const invalidUris = uris.filter((uri) => !uri.startsWith('https://'));
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          {(isLoading || authLoading) ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">No OAuth clients registered yet</p>
              <Button
                variant="outline"
                className="mt-4"
                    ...
                    <TableCell>
                      <code className="text-xs bg-muted px-2 py-1 rounded">
                        {client.client_id.substring(0, 8)}...
                                        ...
                                        <Button variant="ghost" size="icon-sm"  onClick={() => handleCopy(client.client_id, 'client_id')} >
                <Copy className="h-4 w-4" />
                    </Button>
                    </TableCell>
                    <TableCell>
                      <Badge variant={client.is_internal ? 'secondary' : 'default'}>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="form-group">
              ...
              <p className="text-xs text-muted-foreground">
                External apps require HTTPS URIs. Internal apps can use HTTP for development.
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                Available scopes: openid, profile, email, profile.read, email.read, offline_access
              </p>
            </div>
      ...
      <Dialog open={secretDialogOpen} onOpenChange={setSecretDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{newClient ? 'Client Created Successfully' : 'New Client Secret'}</DialogTitle>
            <DialogDescription>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      variant="outline"
                      size="icon"
                      onClick={() => handleCopy(newClient.client_id, 'client_id')}
                    >
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    variant="outline"
                    size="icon"
                    onClick={() => handleCopy(newClient?.client_secret || newSecret || '', 'client_secret')}
                  >
      ...
      <Dialog open={regenerateDialogOpen} onOpenChange={setRegenerateDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to regenerate the secret for "{selectedClient?.name}"? 
