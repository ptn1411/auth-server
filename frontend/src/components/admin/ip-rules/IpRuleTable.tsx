...
import { Globe, Trash2, Loader2, Plus, Shield, ShieldOff } from 'lucide-react';
...
export function IpRuleTable({
  ...
  const { deleteIpRule } = useAdminStore();
  ...
  const handleDeleteIpRule = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteIpRule(selectedIpRule.id);
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Manage system-wide IP whitelist and blacklist rules
              </CardDescription>
            </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-medium mb-2">No global IP rules</h3>
              <p className="text-muted-foreground mb-4">
                Create IP rules to whitelist or blacklist specific IP addresses system-wide.
              </p>
              ...
              <TableBody>
                {ipRules.map((ipRule) => (
                  <TableRow key={ipRule.id}>
                    ...
                    <TableCell className="max-w-[200px] truncate" title={ipRule.reason || undefined}>
                      {ipRule.reason || '-'}
                    </TableCell>
